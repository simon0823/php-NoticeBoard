...
<div class="container mt-4" style="max-width: 1900px">
    <div class="row" style="padding: 0px 70px;">
        <div class="card-deck">
            <!--card-->
            <?php
                $sql = "SELECT board.BoardID, board.MemberID, board.MemberEMAIL, board.BoardTITLE, board.BoardCONTENT, board.BoardDATE, board.BoardVIEW, info.MemberPROFILE FROM account_info AS info JOIN account_board AS board ON info.MemberID = board.MemberID WHERE board.BoardTITLE LIKE :BoardTITLE";
                $stmt = $conn->prepare($sql);
                $stmt->bindParam(':BoardTITLE', $searchKeyword);
                $stmt->execute();

                if ($stmt->rowCount() == 0) {
                    echo "검색한 결과가 존재하지 않습니다!";
                } else {
